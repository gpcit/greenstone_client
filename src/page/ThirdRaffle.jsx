...
const ThirdRaffle = () => {
  const { fetchThirdRaffleWinners, thirdRaffleWinners } = useRaffleWinnerStore();
  ...
  useEffect(() => {
    socket.on("third-raffle-winner-updated", () => {
      fetchThirdRaffleWinners();
    });
    return () => socket.off("third-raffle-winner-updated");
  }, [fetchThirdRaffleWinners]);
  ...
  const balintawakAttendees = thirdRaffleWinners.filter(a => a.isThirdRaffleWinner === true);
  const balintawakChunks = chunkArray(balintawakAttendees, 11);
  return (
    <Layout>
      <div className=' container-fluid mx-auto flex flex-col p-5 lg:p-10'>
        <div >
          <button onClick={() => handleNavigate("/raffle/3rd/qr-scan")} className='bg-green-600 transitions border hover:bg-green-700 text-white font-bold py-2 px-4 rounded mb-5 shadow-lg'>
            Scan QR Code
          </button>
        </div>
        <div className='grid grid-cols-10 gap-4 mb-5'>
          <div className='col-span-10 border rounded-sm border-black shadow-md'>
            <h1 className='text-xl font-bold text-center p-2'>Third Raffle</h1>
                  ...
                  <div className='grid grid-cols-8 gap-2'>
                    {balintawakChunks.map((chunk, idx) => (
                      <div className="flex col-span-4 md:col-span-4 lg:col-span-2 mt-1 gap-4 overflow-x-auto">
                        <Table
                          key={idx}
                          tableHead={thirdRaffleWinnersHead}
                          data={chunk}
